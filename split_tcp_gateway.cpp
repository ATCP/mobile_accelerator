...
void inline bandwidth_probe(u_int tcb_index, u_int sport, u_long_long current_time) {
        
    if (!tcb_table[tcb_index]->last_snd_wnd) {
    ...
    tcb_table[tcb_index]->snd_wnd_count ++;

    if (current_time - tcb_table[tcb_index]->round_start > MIN_RTT/2) {
        tcb_table[tcb_index]->send_rate_upper = tcb_table[tcb_index]->send_rate = 
                (u_long_long)tcb_table[tcb_index]->snd_wnd * 
                (u_long_long)tcb_table[tcb_index]->conn[sport]->max_data_len * 
                (u_long_long)RESOLUTION / (current_time - tcb_table[tcb_index]->round_start);
    }
...
void inline BandwidthAdaptation(u_int tcb_index, u_long_long current_time)
{
    ...
    {        
        ...
        */        
        ...
        if (tcb_table[tcb_index]->RTT > tcb_table[tcb_index]->RTT_limit + DELAY_TOLERANCE) //if (tcb_table[tcb_index]->downlink_queueing_length > 80*MTU)
        {
            ...
            tcb_table[tcb_index]->send_rate = (u_long_long)tcb_table[tcb_index]->rcv_thrughput_approx * (u_long_long)(MIN_RTT + QUEUE_DELAY_TARGET)/ (u_long_long)tcb_table[tcb_index]->RTT;
            ...
            fprintf(stderr, "buffer draining: RTT %u bw %u rate %u mean %u u_size %u uu %d h0 %ld h1 %ld h2 %ld unsent %u u_size %u "
...
#define RATE_THRESHOLD 600000 
                         ...
                         if (tcb_table[tcb_index]->sliding_avg_window.underflow_time >= BUFFER_UNDERFLOW)
                         {
                             ...
                             if (tcb_table[tcb_index]->rcv_thrughput_approx <= tcb_table[tcb_index]->send_rate_lower)
                                 tcb_table[tcb_index]->sliding_avg_window.another_put(tcb_table[tcb_index]->sliding_avg_window.make_up_bw / 
                                        (1.0 *tcb_table[tcb_index]->RTT_limit/SLIDE_TIME_DELTA), current_time, 0);
                             else
                                 tcb_table[tcb_index]->sliding_avg_window.another_put(tcb_table[tcb_index]->sliding_avg_window.make_up_bw / 
                                        (5.0 *tcb_table[tcb_index]->RTT_limit/SLIDE_TIME_DELTA), current_time, 0);
            ...
            fprintf(stderr, "rate adaptation: RTT %u bw %u rate %u mean %u u_size %u uu %d h0 %ld h1 %ld h2 %ld unsent %u u_size %u "
                    "stack %u Q %u nb %u pos %u port %hu\n", 
                    tcb_table[tcb_index]->RTT, 
                    tcb_table[tcb_index]->rcv_thrughput_approx, 
                    tcb_table[tcb_index]->send_rate, 
...
void inline BandwidthAdaptation(u_int tcb_index, u_long_long current_time)
{
    ...
    {        
        ...
        */        
        ...
        if (tcb_table[tcb_index]->downlink_queueing_delay > DELAY_TOLERANCE)
        {
            if (!tcb_table[tcb_index]->sliding_avg_window.upper_heuristic)            
            {
                tcb_table[tcb_index]->sliding_avg_window.upper_heuristic = tcb_table[tcb_index]->sliding_avg_window.heuristic_0;                                       
...
#define UNDERFLOW_THRESHOLD 0
            ...
            {
                if (!tcb_table[tcb_index]->sliding_avg_window._u_size &&  
                        (tcb_table[tcb_index]->downlink_queueing_delay < 0.0008 * RESOLUTION &&
                        tcb_table[tcb_index]->downlink_queueing_length <= UNDERFLOW_THRESHOLD))
                {
                    tcb_table[tcb_index]->sliding_avg_window.lower_heuristic = 0;                     
                    tcb_table[tcb_index]->sliding_avg_window.reload_time = current_time + 
...
#define RATE_THRESHOLD 600000 
            ...
            {                               
                ...
                if ((tcb_table[tcb_index]->downlink_queueing_delay < 0.0008 * RESOLUTION && 
                         tcb_table[tcb_index]->downlink_queueing_length <= UNDERFLOW_THRESHOLD) || 
                        (tcb_table[tcb_index]->downlink_queueing_delay < 0.0008 * RESOLUTION && 
                        tcb_table[tcb_index]->rcv_thrughput_approx < RATE_THRESHOLD))
                {
...
void inline tcb_uplink_bw_est(u_int tcb_index, u_long_long current_time)
{
    ...
    {
        if (tcb_table[tcb_index]->sliding_uplink_window.estmateInterval(current_time) >= SLIDE_TIME_INTERVAL)
        {
            ...
            if (tcb_table[tcb_index]->sliding_uplink_window.k < SLIDE_TIME_INTERVAL/SLIDE_TIME_DELTA)         
                  tcb_table[tcb_index]->sliding_uplink_window.k = SLIDE_TIME_INTERVAL/SLIDE_TIME_DELTA;
        ...
        {            
            ...
            if (tcb_table[tcb_index]->sliding_uplink_window.estmateInterval(current_time) >= 
                tcb_table[tcb_index]->sliding_uplink_window.k*SLIDE_TIME_DELTA)
            {   
                tcb_table[tcb_index]->rcv_uplink_thruput = (tcb_table[tcb_index]->sliding_uplink_window.estmateInterval(current_time) == 0 ? 
                    MAX_UPLINK_BW : (u_long_long)tcb_table[tcb_index]->sliding_uplink_window.bytes() * 
                        (u_long_long)RESOLUTION / (u_long_long)tcb_table[tcb_index]->sliding_uplink_window.estmateInterval(current_time));
                ...
                tcb_table[tcb_index]->rcv_uplink_thruput_approx = (1 - double(tcb_table[tcb_index]->sliding_uplink_window.k* SLIDE_TIME_DELTA)/ 
                        double(SLIDE_TIME_INTERVAL)) * MAX_UPLINK_BW + (double(tcb_table[tcb_index]->sliding_uplink_window.k* SLIDE_TIME_DELTA)/ 
                        double(SLIDE_TIME_INTERVAL)) * tcb_table[tcb_index]->rcv_uplink_thruput;
...
void inline tcb_bw_burst_ctrl(u_int tcb_index, u_long_long current_time, u_int sport)
{
    ...
    {        
        tcb_table[tcb_index]->sliding_avg_window.shiftWinByTime(current_time);
	...
	{           
            ...
            */
            ...
            if (tcb_table[tcb_index]->sliding_avg_window.k < SLIDE_TIME_INTERVAL/SLIDE_TIME_DELTA)         
            {
	...
	else if (tcb_table[tcb_index]->sliding_avg_window.estmateInterval(current_time) < SLIDE_TIME_INTERVAL)
	{
            
                       
            if (tcb_table[tcb_index]->sliding_avg_window.estmateInterval(current_time) >= tcb_table[tcb_index]->sliding_avg_window.k * SLIDE_TIME_DELTA)
            {
		...
		tcb_table[tcb_index]->rcv_thrughput = (tcb_table[tcb_index]->sliding_avg_window.estmateInterval(current_time) == 0 ? 
                    tcb_table[tcb_index]->send_rate_upper : (u_long_long)tcb_table[tcb_index]->sliding_avg_window.bytes() * (u_long_long)RESOLUTION / 
                        (u_long_long)tcb_table[tcb_index]->sliding_avg_window.estmateInterval(current_time));
                
		tcb_table[tcb_index]->rcv_thrughput_approx =  (1 - double(tcb_table[tcb_index]->sliding_avg_window.k* SLIDE_TIME_DELTA)/ double(SLIDE_TIME_INTERVAL)) * 
                        tcb_table[tcb_index]->send_rate_upper + (double(tcb_table[tcb_index]->sliding_avg_window.k* SLIDE_TIME_DELTA)/ double(SLIDE_TIME_INTERVAL)) * 
                        tcb_table[tcb_index]->rcv_thrughput;
		...
		BandwidthAdaptation(tcb_index, current_time);                
                ...
                /*
                if (tcb_table[tcb_index]->unsent_size)
                {
                    if (rcv_thruput > tcb_table[tcb_index]->send_rate_upper)
                    {
                        if (max_bytes > bytes)
                        {
                            tcb_table[tcb_index]->sliding_avg_window.pop(max_bytes - bytes);
                            tcb_table[tcb_index]->unsent_size -= (max_bytes - bytes); 
                        }  
                    }
                    else
                    {
                        tcb_table[tcb_index]->sliding_avg_window.pop(tcb_table[tcb_index]->unsent_size);
                        tcb_table[tcb_index]->unsent_size = 0;
                    }
                }
                */                        
...
void inline update_busy_period(u_short sport, u_int tcb_index, u_int ack_num, u_long_long current_time, u_short ctr_flag)
{
    ...
    tcb_table[tcb_index]->BusyPeriod._it = tcb_table[tcb_index]->BusyPeriod._head;

    for (int i = tcb_table[tcb_index]->BusyPeriod._size; i > 0; i --)
    {                     
        if (!tcb_table[tcb_index]->BusyPeriod.it()->started && tcb_table[tcb_index]->BusyPeriod.it()->start_dport == sport && MY_SEQ_LEQ(tcb_table[tcb_index]->BusyPeriod.it()->start_seq_no, ack_num))
        {
            ...
            if (!tcb_table[tcb_index]->sliding_avg_window.sample_time)
            {
                ...
                printf("Sample time: %llu Busy Period Size: %u Started: %u at ACK: %u %u------------------------\n", tcb_table[tcb_index]->sliding_avg_window.sample_time, tcb_table[tcb_index]->BusyPeriod._size, tcb_table[tcb_index]->BusyPeriod.head()->started, ack_num, tcb_table[tcb_index]->BusyPeriod.it()->start_seq_no);
            }
            else
            {                                        
                if (tcb_table[tcb_index]->sliding_avg_window.tailTime())
                {                        
                    tcb_table[tcb_index]->sliding_avg_window.updateShiftTime((current_time <= tcb_table[tcb_index]->sliding_avg_window.tailTime() ? 0 : current_time - tcb_table[tcb_index]->sliding_avg_window.tailTime()));


                }
                else
                {                        
                    tcb_table[tcb_index]->sliding_avg_window.updateShiftTime((current_time <= tcb_table[tcb_index]->sliding_avg_window.tailTime() ? 0 : current_time - tcb_table[tcb_index]->sliding_avg_window.sample_time)); 
            ...
            tcb_table[tcb_index]->BusyPeriod.it()->started = TRUE;

            while (tcb_table[tcb_index]->BusyPeriod._head != tcb_table[tcb_index]->BusyPeriod._it)
            {
                
                tcb_table[tcb_index]->BusyPeriod.head()->init();                                          
                tcb_table[tcb_index]->BusyPeriod.headNext();
                tcb_table[tcb_index]->BusyPeriod.decrease();

            }


            printf("Start Shift time: %u Busy Period Size: %u Started: %u at %u %u---------\n", tcb_table[tcb_index]->sliding_avg_window.shift_time, tcb_table[tcb_index]->BusyPeriod._size, tcb_table[tcb_index]->BusyPeriod.head()->started, ack_num, tcb_table[tcb_index]->BusyPeriod.it()->start_seq_no);
            fprintf(tcb_table[tcb_index]->out_file, "Start Shift time: %u Busy Period Size: %u Started: %u at %u %u---------\n", tcb_table[tcb_index]->sliding_avg_window.shift_time, tcb_table[tcb_index]->BusyPeriod._size, tcb_table[tcb_index]->BusyPeriod.head()->started, ack_num, tcb_table[tcb_index]->BusyPeriod.it()->start_seq_no);
            
        }                        


        if ((tcb_table[tcb_index]->BusyPeriod.it()->end_dport == sport && MY_SEQ_LEQ(tcb_table[tcb_index]->BusyPeriod.it()->end_seq_no, ack_num)) || ((ctr_flag & 0x04) == 4 /*&& (tcb_table[tcb_index]->BusyPeriod.it()->end_dport == sport || tcb_table[tcb_index]->BusyPeriod.it()->start_dport == sport)*/))
        {                   
            ...
            printf("End shift time %u Busy Period Size: %u Started: %u at %u %u-----------\n", tcb_table[tcb_index]->sliding_avg_window.shift_time, tcb_table[tcb_index]->BusyPeriod._size, tcb_table[tcb_index]->BusyPeriod.head()->started, ack_num, tcb_table[tcb_index]->BusyPeriod.it()->end_seq_no);

        }

        tcb_table[tcb_index]->BusyPeriod.itNext();
    }
        
        
}
void inline denote_busy_peirod_start(u_int tcb_index, u_short dport, u_int seq_num)
{

    if (check_buffer_empty(tcb_index, dport))
    {      
        /*
        while(tcb_table[tcb_index]->BusyPeriod.tail()->start_seq_no && tcb_table[tcb_index]->BusyPeriod.size() < tcb_table[tcb_index]->BusyPeriod.capacity) 
             tcb_table[tcb_index]->BusyPeriod.tailNext();
        */

        tcb_table[tcb_index]->BusyPeriod.increase();
        tcb_table[tcb_index]->BusyPeriod.tail()->start_dport = dport;
        tcb_table[tcb_index]->BusyPeriod.tail()->start_seq_no = seq_num;
                
        if (tcb_table[tcb_index]->conn[dport]->server_state.phase == FAST_RTX)
        {
            printf("FAST RTX Denote Busy Period start, %u %u %u %u--------------------------------------------------\n", tcb_table[tcb_index]->BusyPeriod.tail()->start_dport, tcb_table[tcb_index]->BusyPeriod.tail()->start_seq_no, tcb_table[tcb_index]->BusyPeriod._size, tcb_table[tcb_index]->BusyPeriod.head()->started, tcb_table[tcb_index]->BusyPeriod.head()->end_seq_no);   
        }
        else
        {
            printf("NORMAL Denote Busy Period start, %u %u %u %u %u--------------------------------------------------\n", tcb_table[tcb_index]->BusyPeriod.tail()->start_dport, tcb_table[tcb_index]->BusyPeriod.tail()->start_seq_no, tcb_table[tcb_index]->BusyPeriod._size, tcb_table[tcb_index]->BusyPeriod.head()->started, tcb_table[tcb_index]->BusyPeriod.head()->end_seq_no);        
        }        
    }
    
}
void inline denote_busy_period_end(u_int tcb_index, u_short sport, u_int seq_num, BOOL transmitting, u_long_long current_time)
{       
        
    if (check_buffer_empty(tcb_index, sport))
    {                                                        
        tcb_table[tcb_index]->BusyPeriod.tail()->end_dport = sport;
        tcb_table[tcb_index]->BusyPeriod.tail()->end_seq_no = seq_num;
        tcb_table[tcb_index]->BusyPeriod.tailNext();
    }
    else
    {
        if (transmitting)
        {
            ...
            if (!tcb_table[tcb_index]->BusyPeriod.tail()->start_dport)
            {
                tcb_table[tcb_index]->BusyPeriod.increase();
                tcb_table[tcb_index]->BusyPeriod.tail()->start_dport = sport;
                tcb_table[tcb_index]->BusyPeriod.tail()->start_seq_no = seq_num;
                tcb_table[tcb_index]->BusyPeriod.tail()->idle_end_time = current_time;
                
                if (tcb_table[tcb_index]->BusyPeriod.tail()->idle_start_time)
                {
                    tcb_table[tcb_index]->unsent_data_bytes += (tcb_table[tcb_index]->BusyPeriod.tail()->idle_end_time - tcb_table[tcb_index]->BusyPeriod.tail()->idle_start_time) * tcb_table[tcb_index]->rcv_thrughput / RESOLUTION;
                    
                    fprintf(tcb_table[tcb_index]->out_file, "---%u---%llu---%llu---%u---\n", tcb_table[tcb_index]->unsent_data_bytes, tcb_table[tcb_index]->BusyPeriod.tail()->idle_end_time, tcb_table[tcb_index]->BusyPeriod.tail()->idle_start_time, tcb_table[tcb_index]->send_rate);
                }
                
                
                if (tcb_table[tcb_index]->conn[sport]->server_state.phase == FAST_RTX)
                {                     
                    printf("FAST RTX idle Denote Busy Period start, %u %u %u %u-------------------\n", tcb_table[tcb_index]->BusyPeriod.tail()->start_dport, tcb_table[tcb_index]->BusyPeriod.tail()->start_seq_no, tcb_table[tcb_index]->BusyPeriod._size, tcb_table[tcb_index]->BusyPeriod.head()->started, tcb_table[tcb_index]->BusyPeriod.head()->end_seq_no);   
                     
                }                
                else
                {
                     
                    printf("NORMAL idle Denote Busy Period start, %u %u %u %u %u------------------\n", tcb_table[tcb_index]->BusyPeriod.tail()->start_dport, tcb_table[tcb_index]->BusyPeriod.tail()->start_seq_no, tcb_table[tcb_index]->BusyPeriod._size, tcb_table[tcb_index]->BusyPeriod.head()->started, tcb_table[tcb_index]->BusyPeriod.head()->end_seq_no);        
        ...
        else
        {
            if (tcb_table[tcb_index]->BusyPeriod.tail()->start_dport)   
            {
                ...
                if (tcb_table[tcb_index]->conn[sport]->server_state.phase == FAST_RTX)
                {
                     printf("FAST RTX idle Denote Busy Period end, %u %u %u %u-------------------\n", tcb_table[tcb_index]->BusyPeriod.tail()->end_dport, tcb_table[tcb_index]->BusyPeriod.tail()->end_seq_no, tcb_table[tcb_index]->BusyPeriod._size, tcb_table[tcb_index]->BusyPeriod.head()->started, tcb_table[tcb_index]->BusyPeriod.head()->end_seq_no);   

                }
                else
                {

                     printf("NORMAL idle Denote Busy Period end, %u %u %u %u %u------------------  \n", tcb_table[tcb_index]->BusyPeriod.tail()->end_dport, tcb_table[tcb_index]->BusyPeriod.tail()->end_seq_no, tcb_table[tcb_index]->BusyPeriod._size, tcb_table[tcb_index]->BusyPeriod.head()->started, tcb_table[tcb_index]->BusyPeriod.head()->end_seq_no);    

                }
                
                tcb_table[tcb_index]->BusyPeriod.tailNext();
                tcb_table[tcb_index]->BusyPeriod.tail()->idle_start_time = current_time;
            }
        }
    }
...
int inline nxt_schedule_tcb()
{
	...
	while (!pool.ex_tcb.isEmpty())
	{
            ...
            else
            {
                ...
                timeInterval = (double)tcb_table[tcb_index]->sliding_snd_window.bytes() * 
                        (double)RESOLUTION / 
    ...
    while (!tcb_table[tcb_index]->states.isEmpty())
    {
        ...
        else
        {
            ...
            else
            {
                ...
                //tcb_table[tcb_index]->conn[sport]->sliding_avg_window.updateShiftTime(timeSleep < 0 ? 0-timeSleep:0);
                ...
                {                    
                    ...
                    if (tcb_table[tcb_index]->conn[sport]->server_state.phase == NORMAL)
                    {
                        ...
                        else if (tcb_table[tcb_index]->conn[sport]->server_state.ignore_adv_win)
                        {
                            ...
                            snd_win = tcb_table[tcb_index]->conn[sport]->max_data_len * tcb_table[tcb_index]->snd_wnd;
                        ...
                        if (timeoutPkt->snd_time && current_time >= timeoutPkt->snd_time + tcb_table[tcb_index]->conn[sport]->rto*2)
                        {
                                ...
                                */
                                ...
                                if (tcb_table[tcb_index]->conn[sport]->client_state.rcv_wnd == 0)
                                    send_ack_back(sport, data,  tcb_table[tcb_index]->conn[sport]->client_ip_address, 
                                                   ...
                                                   sport, tcb_table[tcb_index]->conn[sport]->sPort, 
                                                   tcb_table[tcb_index]->conn[sport]->client_state.snd_nxt, 
                                                   tcb_table[tcb_index]->conn[sport]->client_state.rcv_nxt, 
                                                   0 |16, 0, 
                                                   tcb_table[tcb_index]->conn[sport]->client_state.send_data_id + 1, 
