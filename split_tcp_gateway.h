...
#define CIRCULAR_BUF_SIZE 2048*10
...
#define DELAY_TOLERANCE 1000000
#define DELAY_THRES 0.0006
#define DELAY_STDDEV 100000

#define QUEUE_THRES 1
#define QUEUE_DELAY_TARGET 50000
...
struct SlideWindow
{
	...
	u_long_long sample_time;
        ...
        u_long_long sent_timestamp_rep;
        u_long_long last_timestamp_rep;
...
#define HEURISTIC_UNSENT_STACK_SIZE 0
	...
	void another_shift()
	{
            while (frontTime() < sample_time && _bytes > 0 && _size >= 3)
            {
                shift();
                _size --;
            }
	...
	void another_put(u_int len, u_long_long time, u_int seqNo)
	{
        ...
        inline Packet* uhead() { return unsent_window + (_u_head % capacity); }
	inline void uheadNext() { _u_head = (_u_head + 1) % capacity; _u_size --; }
        inline Packet* utail() { return unsent_window + (_u_tail % capacity); }
        inline void utailNext() { _u_tail = (_u_tail + 1) % capacity; _u_size ++; }
...
#define PARAMETER_TUNING     
#define NB_RATE_UPDATE 10
        
        void window_update(u_long_long current_time, u_int rtt, u_int rtt_limit, u_int queue_len, u_int rcv_bw, u_long_long now)
        {                
            while (_u_size > 0) 
            {                    
                ...
                if (MY_SEQ_GEQ(current_time, unsent_data->future_time))
                {
                    ...
                    if (unsent_data->sign)
                    {
                        unsent_queue.push_back(unsent_data->unsent_len);
                    }
                    else
                    {
                        unsent_queue.push_back((-1)*unsent_data->unsent_len);
                    }
...
#else
                        #define NB_COMPENSATION 5
                        for (int i = 0; i < NB_COMPENSATION; i ++)
                        {
                            if (unsent_queue.size()) {
                                counted_unsent_bytes += unsent_queue.front();
                                if (counted_unsent_bytes < 0)
                                    counted_unsent_bytes = 0;  

                                if (unsent_queue.front() > 0)
                                    record_unsent_pos(now);
                                else
                                {
                                    if (nb_unsent_pos >= unsent_nb_pos(rcv_bw)) 
                                    {
                                        update_unsent_pos(counted_unsent_bytes / nb_unsent_pos);
                                        fprintf(stderr, "\n\n\n\n\n",nb_unsent_pos);                            
                                    }
                                }

                                unsent_queue.pop_front();                            
                            }
                            else 
                                break;
                        }
                    ...
                    {   
                        ...
                        if (_u_size < unsent_cap) {                        
                            ...
                            return_data->unsent_len = unsent_data->unsent_len;
                            return_data->sign = unsent_data->sign;

                            if (sent_timestamp_rep <= last_timestamp_rep) {
        ...
        void shiftWinByTime(u_long_long current_time)
        {            
            u_long_long shift = 0;
            if (shift_time)
            {                                   
                if (shift_time > (current_time > tailTime() ? current_time - tailTime() : 0))
                {
                    shift = (current_time > tailTime() ? current_time - tailTime() : 0);
                    shift_time -= (current_time > tailTime() ? current_time - tailTime() : 0);
                }
                else
                {
                    shift = shift_time;
        ...
        void updateShiftTime(u_long_long time)
        {
            shift_time += time;
        }
